...
if __name__ == "__main__":
    ...
    # solver_array = ["gmres"]
    # solver_array = ["gcrodr"]
    data_path_1 = './recycle datasets/Poisson_generate/data/data_poisson_0.005_50_1_182681.mat'
    data_path_2 = './recycle datasets/Poisson_generate/data/data_poisson_0.005_50_2_182681.mat'
    ###################################################################################################################
    ...
    # 从matlab得到数据集
    data = scipy.io.loadmat(data_path_1)
    A1 = [item[0] if isinstance(item, list) and len(item) == 1 else item for item in data['A_data'][0]]
    b1 = data['b_data']
    w1 = [item[0] if isinstance(item, list) and len(item) == 1 else item for item in data['w_data'][0]]
    data = scipy.io.loadmat(data_path_2)
    A2 = [item[0] if isinstance(item, list) and len(item) == 1 else item for item in data['A_data'][0]]
    b2 = data['b_data']
    w2 = [item[0] if isinstance(item, list) and len(item) == 1 else item for item in data['w_data'][0]]
