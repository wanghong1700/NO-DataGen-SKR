...
def seq_generator(dataset, size_mat, num_data, rel_path, prm_freq, dim_pca, w, r):
    ...
    for i0 in range(num_data - 1):
        ...
        j = 0
        for j0 in lst:
            dis0 = comp_frobenius_distance(w[i], w[j0]) + abs(r[i] - r[j0])
...
if __name__ == "__main__":
    ...
    # 本次运算使用的相关参数
    # 实验主题
    theme = '不同误差下 不同预处理下 贪心排序 elec 的 两种算法对比'
    exp_id = '20230919_4'
    # 所用数据集
    dataset = 'elec_mpi'
    # 数据集的大小
    ...
    data_path = '../recycle datasets/Elsc_generate/data/data_elec_0.006_100_25804.mat'
    ###################################################################################################################
